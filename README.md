...
- **No LLM in the v1.0 hot path.** The 14-rule matcher covers three Cucumber dialects (first-person, third-person, no-subject) plus compound input steps. Genuinely vague steps are inherently ambiguous and get `// TODO` — better than an LLM guessing.
...
└──────┬───────┘
       ▼
┌──────────────┐
│ stepMatcher  │  14 rules — subject-agnostic, dialect-tolerant,
