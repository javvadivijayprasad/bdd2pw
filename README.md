...
- **No LLM in the v1.0 hot path.** The 15-rule matcher (14 in v1.0, +1 URL-contains in v1.0.1) covers three Cucumber dialects (first-person, third-person, no-subject) plus compound input steps. Genuinely vague steps are inherently ambiguous and get `// TODO` — better than an LLM guessing.
...
└──────┬───────┘
       ▼
┌──────────────┐
│ stepMatcher  │  15 rules — subject-agnostic, dialect-tolerant,
