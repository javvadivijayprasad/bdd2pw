...
| **Phase 1a** | ✅ Shipped | Rule-based step matcher (22 rules — 14 in v1.0, +1 URL-contains in v1.0.1, +7 LLM-narrative dialect rules in v1.1.1), Gherkin parser, repo scanner, locator picker, file-snapshot discovery, end-to-end `scaffold()` against `practice-test-login` (7/7, 0 warnings, locked as regression). |
...
- **Rule taxonomy: 22 deterministic patterns + compound `customBody`** (14 at v1.0, +1 URL-contains in v1.0.1, +7 LLM-narrative dialects in v1.1.1) — locked
